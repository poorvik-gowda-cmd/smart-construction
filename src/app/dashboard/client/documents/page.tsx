    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">View files shared by your engineer and upload your own plans or photos.</p>
        </div>
        <button
        ...
        >
          <Plus className="w-5 h-5 mr-2" /> Upload Document
        </button>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          {fromEngineer.length > 0 && (
            <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-xl">
              <div className="p-5 border-b border-white/5 flex items-center space-x-3">
                ...
                <h2 className="text-base font-bold text-white">From Your Engineer</h2>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-xl">
            ...
            {myUploads.length === 0 ? (
              <div className="py-12 text-center text-slate-600 text-sm">No uploads yet. Use the button above to share plans or photos.</div>
            ) : (
              <div className="divide-y divide-white/5">
                {myUploads.map(doc => (
                  <a key={doc.id} href={doc.file_url} target="_blank" rel="noreferrer"
          ...
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleUpload} className="space-y-4">
              <div>
                ...
                <input type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full mt-1 bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500 transition-colors" placeholder="e.g., Site Plan v2" />
              </div>
              <div>
              ...
              <p className="text-[10px] text-slate-600">Your uploaded document will be visible to your assigned engineer and the admin team.</p>
