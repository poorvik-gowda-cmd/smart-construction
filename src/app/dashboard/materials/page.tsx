  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">{t('Material Inventory')}</h1>
          <p className="text-slate-500 mt-1">{t('Track stocks, manage reorders, and monitor consumption across sites.')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-lg group hover:border-blue-900/50 transition-colors cursor-pointer">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold text-slate-500 bg-slate-950 px-2 py-1 rounded shadow-inner uppercase tracking-widest leading-none">{t('Total Items')}</span>
            </div>
            <p className="text-2xl font-extrabold text-slate-100 italic tracking-tight">{materials.reduce((acc, m) => acc + (m.stock_level || 0), 0).toLocaleString()} <span className="text-slate-500 text-xs font-medium not-italic ml-1 uppercase">{t('Units')}</span></p>
          </div>
          ...
          <div className="bg-slate-900 border border-rose-900/40 p-6 rounded-2xl space-y-4 shadow-lg group hover:border-rose-900 transition-colors cursor-pointer animate-pulse-slow">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold text-rose-500 bg-rose-500/5 px-2 py-1 rounded shadow-inner uppercase tracking-widest leading-none">{t('Critical Stock')}</span>
            </div>
            <p className="text-2xl font-extrabold text-slate-100 italic tracking-tight">{materials.filter(m => (m.stock_level || 0) <= (m.reorder_point || 0)).length} <span className="text-rose-500 text-xs font-medium not-italic ml-1 uppercase">{t('Below Reorder')}</span></p>
          </div>
         ...
         <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-lg group hover:border-emerald-900/50 transition-colors cursor-pointer">
            <div className="flex items-center justify-between">
               ...
               <span className="text-[10px] font-bold text-slate-500 bg-slate-950 px-2 py-1 rounded shadow-inner uppercase tracking-widest leading-none">{t('Weekly Arrivals')}</span>
            </div>
            <p className="text-2xl font-extrabold text-slate-100 italic tracking-tight">24 <span className="text-emerald-500 text-xs font-medium not-italic ml-1 uppercase">{t('Consignments')}</span></p>
         </div>
         ...
         <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-lg group hover:border-amber-900/50 transition-colors cursor-pointer">
            <div className="flex items-center justify-between">
               ...
               <span className="text-[10px] font-bold text-slate-500 bg-slate-950 px-2 py-1 rounded shadow-inner uppercase tracking-widest leading-none">{t('Burn Rate')}</span>
            </div>
            <p className="text-2xl font-extrabold text-slate-100 italic tracking-tight">6.2% <span className="text-amber-500 text-xs font-medium not-italic ml-1 uppercase">{t('Monthly Inc.')}</span></p>
         </div>
      </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-950/50 border-b border-white/5">
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">{t('Material Resource')}</th>
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">{t('Project Usage')}</th>
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">{t('Current Stock')}</th>
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">{t('Status')}</th>
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">{t('Actions')}</th>
              </tr>
            </thead>
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-500 font-bold uppercase mt-0.5 tracking-widest">{t('Unit:')} {material.unit}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex items-center text-slate-400 text-xs font-bold uppercase tracking-widest">
                        {t('Project Site')} #{material.project_id}
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="space-y-1.5">
                        <div className="flex items-baseline justify-between">
                           ...
                           <span className="text-[10px] text-slate-500 font-medium">{t('Reorder at')} {reorderPoint}</span>
                        </div>
                    ...
                    <td className="px-6 py-5">
                      {isCritical ? (
                        <div className="flex items-center text-rose-500 space-x-2">
                           ...
                           <span className="text-[10px] font-bold uppercase tracking-widest">{t('Low Stock Alert')}</span>
                        </div>
                      ) : (
                        <div className="flex items-center text-emerald-500 space-x-2">
                           <ArrowUpCircle className="w-4 h-4" />
                           <span className="text-[10px] font-bold uppercase tracking-widest">{t('Stock Stable')}</span>
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-white">{t('Add Inventory Item')}</h3>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('Material Name')}</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('Unit')}</label>
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('Current Stock')}</label>
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('Reorder Point Alert')}</label>
